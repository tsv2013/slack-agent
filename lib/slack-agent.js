...
SlackAgent.prototype._isFromSlackAgent = function (message) {
    return message.user === this.user.id;
};

SlackAgent.prototype._isMentioningSlackAgent = function (message) {
    return message.text.toLowerCase().indexOf('agent') > -1 ||
...
SlackAgent.prototype._reply = function (originalMessage) {
    var self = this;
    ...
    // });
    var estimation = analyze(originalMessage.text);
    var channel = self._getChannelById(originalMessage.channel);
    self.postMessageToChannel(channel.name, "uups!" + JSON.stringify(estimation), { as_user: true });
};

SlackAgent.prototype._onMessage = function (message) {
    if(this._isChatMessage(message) &&
        this._isChannelConversation(message) &&
        !this._isFromSlackAgent(message) &&
        this._isMentioningSlackAgent(message)
    ) {
        this._reply(message);
    }
};
